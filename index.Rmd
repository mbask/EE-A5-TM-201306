...
countryAggrData_l <- lapply(
  countryAggrData_l
  , function(country_df) { tmp_df <- ddply(country_df, .(parameterName), summarize, Count = length(parameterName)); colnames(tmp_df) <- c("Parameter Name", "Count"); tmp_df}
  )
...
plot(
  gvisMap(
    ...
    , chartid = "CountryPlotPoland"
    )
  )

print(
  xtable(
    head(countryAggrData_l[[1]][with(countryAggrData_l[[1]], order(-Count)), ], 10))
  , type = 'html'
